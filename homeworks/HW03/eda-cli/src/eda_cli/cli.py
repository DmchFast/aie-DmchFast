...
def report(
    ...
    # 4. Markdown-отчёт с использованием новых параметров
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if not missing_df.empty:
            problem_columns = missing_df[missing_df["missing_share"] > min_missing_share]
            if not problem_columns.empty:
                ...
                for col_name, row in problem_columns.iterrows():
        ...
        if not top_cats:
            ...
            f.write(f"Top-{top_k_categories} значений по категориальным признакам:\n\n")
            for col_name, table in top_cats.items():
                f.write(f"### {col_name}\n\n")
                f.write(f"Всего уникальных значений: {df[col_name].nunique(dropna=True)}\n\n")
                # Выводим таблицу в markdown формате
