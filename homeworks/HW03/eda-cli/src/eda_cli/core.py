...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in summary.columns:
        if col.unique == 1 and col.missing == 0:  # Все значения одинаковые и нет пропусков
            constant_columns.append(col.name)
    flags["has_constant_columns"] = len(constant_columns) > 0
    flags["constant_columns"] = constant_columns
    
    #--- Новая эвристика 2: Категориальные колонки с высокой кардинальностью ---
    ...
    # Простейший «скор» качества (доработанный с учетом новых эвристик)
    score = 1.0
